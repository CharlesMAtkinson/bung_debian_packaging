...
function build {
    ...
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    msg I "Changing to $bung_ver_dir"
    cd "$bung_ver_dir" || finalise 1
    ...
    do
       files_list+="bung_$new_pack_full_ver$part_name "
    done
    ...
    msg I "Build output files"$'\n'"$(ls -l "$save_dir/bung_$new_pack_full_ver"*)"
...
function initialise {
    ...
    # Derive versions
    # ~~~~~~~~~~~~~~~
    new_bung_ver=${new_pack_full_ver%-*}
    ...
    [[ $buf != '' ]] && msg E "$buf" 
    grep --fixed-strings --quiet "bung ($new_pack_full_ver) " "$fn"
    (($?!=0)) && msg E "$fn is not for the requested package.  Content:"$'\n'"$(<"$fn")"

    # Is the source tarball present?
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    source_tarball_fn=source/bung_$new_bung_ver.orig.tar.gz
...
function populate_tmp_dir {
    ...
    # |
    # +-- bung_$bung_ver.orig.tar.gz

    # Create the bung_$bung_ver drirectory
    # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    bung_ver_dir=$tmp_dir/bung_$new_bung_ver
    mkdir "$bung_ver_dir" || finalise 1

    # Copy the source tarball
    # ~~~~~~~~~~~~~~~~~~~~~~~~~
    dest_fn=$tmp_dir/bung_$new_bung_ver.orig.tar.gz
    msg I "Copying $source_tarball_fn to $dest_fn"
    cp -p "$source_tarball_fn" "$dest_fn" || finalise 1

    # Extract the tarball
    # ~~~~~~~~~~~~~~~~~~~
    msg I "Extracting tarball $source_tarball_fn to $bung_ver_dir"
    cmd=(tar --extract --file "$source_tarball_fn" --directory "$bung_ver_dir")
    buf=$("${cmd[@]}" 2>&1)
    rc=$?
    if ((rc!=0)) || [[ $buf != '' ]]; then
        msg=$'\n'"Command: ${cmd[*]}"
    ...
    cp -pr debian "$bung_ver_dir/debian"
